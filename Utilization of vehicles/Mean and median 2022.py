...
utilize = new_data.query("year == '2022'").groupby(["day_of_week"], as_index=False)["day_of_week"].value_counts()\
...
data = []
monday = np.random.uniform(0, 1, 4437)
for i in monday:
    data.append(i)
tuesday = np.random.uniform(1, 2, 2865)
for i in tuesday:
    data.append(i)
Wednesday = np.random.uniform(2, 3, 5925)
for i in Wednesday:
    data.append(i)
Thursday = np.random.uniform(3, 4, 442)
for i in Thursday:
    data.append(i)
Friday = np.random.uniform(4, 5, 90)
for i in Friday:
    data.append(i)
Saturday = np.random.uniform(5, 6, 1590)
for i in Saturday:
    data.append(i)
Sunday = np.random.uniform(6, 7, 704)
for i in Sunday:
    data.append(i)
